...
//Intento de IA que no esta listo
#include <stdio.h>
#include <time.h>
#include <stdlib.h>
#include <math.h>
...
int casos(int bando){
	switch(bando){
	case 1:
		// return tablero si empieza arriba
	case 2:
		//return tablero si empieza abajo
	}
}
...
//char tableroScore[16][16]{
	//hacer tablero de puntos para comparar con los movimientos
//}
...
void juegoPurete(char *nombre,unsigned int colorJugador, unsigned int turno){
  ...
  while(victoria == 0){
    	...
    	if(turno != PERSONA){
			...
			int cont=0;
    		do{
    		  //ver para que recorra si o si todas las piezas y que no este revisando la misma pieza
			  pcSiguiente.x = coinFlip(15);
			  pcSiguiente.y = coinFlip(15);
			  pcActual.x = coinFlip(15);
			  pcActual.y = coinFlip(15);
			  //verifica primero si puede realizar un salto
			  if(tablero[pcActual.x][pcActual.y] == (colorJugador = NEGRO?'B':'N')){
				  bandera =  checkSalto(pcActual,pcSiguiente);
			  	  cont++;
			  	  //se puede realizar un salto
			  	  if (bandera != 0){
			  		  while (bandera != 0){
			  			//se asignan los lugares del salto como casilla incial
			  			pcActual.x = pcSiguiente.x; ///////////////////hacer con axuliares para no perder el salto
			  			pcActual.y = pcSiguiente.y;
			  			//se selecciona nuevo lugar para saltar
			  			pcSiguiente.x = coinFlip(15);
			  			pcSiguiente.y = coinFlip(15);
			  			if(tablero[pcActual.x][pcActual.y] == (colorJugador = NEGRO?'B':'N')){
			  				//verifica si se puede realizar un segundo salto
			  				bandera =  checkSalto(pcActual,pcSiguiente);
			  				//si es que si se puede realizar otro salto se realiza el salto anterior en el tablero
			  				if (bandera != 0){
			  					tablero[pcActual.x][pcActual.y] = '-';
			  					tablero[pcSiguiente.x][pcSiguiente.y] = (colorJugador = NEGRO?'B':'N');
			  				}else{
			  					...
			  					break;
			  					//aunque creo que sale antes porque cheeckSalto retorna 0
			  				}
			  			}
			  		}
			  	  }
    		...
    		do{
    		   //ver para que recorra si o si todas las piezas y que no este revisando la misma pieza
    			...
    			//verifica primero si puede realizar un salto
    			if(tablero[pcActual.x][pcActual.y] == (colorJugador = NEGRO?'B':'N')){
    				bandera =  checkadyacente(pcActual,pcSiguiente);
    				cont++;
    			}
    			if (cont>19){
    				break;
